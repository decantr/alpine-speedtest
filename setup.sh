...
# add php-deps
apk add --no-cache --virtual=php-dependencies \
	composer \
	php84-fileinfo \
	php84-fpm \
	php84-gd \
...
if [ -z ${CLI_VERSION+x} ]; then
	CLI_VERSION=$(
		curl -Ls https://packagecloud.io/ookla/speedtest-cli/debian/dists/bookworm/main/binary-amd64/Packages |
...
echo ":: configure php-fpm to pass env var"
...
cp config.nginx /etc/nginx/http.d/default.conf

chown -R nobody:nobody "$APP_PATH"

service nginx start
service php-fpm84 start
